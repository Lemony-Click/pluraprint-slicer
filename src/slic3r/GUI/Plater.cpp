...
void Sidebar::priv::layout_printer(bool isBBL, bool isDual)
{
    ...
    // ref: https://github.com/PluraprintSlicer/PluraprintSlicer/pull/11610#discussion_r2607411847
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            _L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update Pluraprint Slicer or restart Pluraprint Slicer to check if there is an update to system presets.") + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if(load_type != LoadType::LoadGeometry)
                            show_info(q, _L("The 3MF is not supported by PluraprintSlicer, loading geometry data only."), _L("Load 3MF"));
                    }
                    ...
                    //             show_info(q, _L("The 3MF was generated by an old PluraprintSlicer, loading geometry data only."), _L("Load 3MF"));
                    //         else
                    //             show_info(q, _L("The 3MF is not supported by PluraprintSlicer, loading geometry data only."), _L("Load 3MF"));
                    //     }
                    ...
                    //     }
                    // }
                    // Orca: check if the project is created with PluraprintSlicer 2.3.1-alpha and use the sparse infill rotation template for non-safe infill patterns
                        ...
                        if (!config_loaded.opt_string("sparse_infill_rotate_template").empty()) {
                            ...
                            if (!is_safe_to_rotate) {
                                wxString msg_text = _(
                                    L("This project was created with an PluraprintSlicer 2.3.1-alpha and uses "
                                ...
                                msg_text += "\n\n" +
                                            _(L("Would you like PluraprintSlicer to automatically fix this by clearing the rotation template settings?"));
                        ...
                        load_config = false;
                        show_info(q, _L("The 3MF file was generated by an old PluraprintSlicer version, loading geometry data only."), _L("Load 3MF"));
                    }
                    else if (!load_config) {
...
void Plater::priv::set_project_name(const wxString& project_name)
{
    ...
    //update topbar title
#ifdef __WINDOWS__
    wxGetApp().mainframe->SetTitle(m_project_name + " - PluraprintSlicer");
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Pluraprint Slicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = _L("Importing to Pluraprint Slicer failed. Please download the file and manually import it.");
                        cont = false;
                    }
                })
    ...
    , m_action(2)
{
    ...
    // icon
    std::string icon_path = (boost::format("%1%/images/PluraprintSlicerTitle.ico") % resources_dir()).str();
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("PluraprintSlicer can't connect to %s. Please check if the printer is powered on and connected to the network."), printer_name);
        }
...
void Plater::show_object_info()
{
    ...
    if (non_manifold_edges > 0) {
        info_manifold += into_u8("\n" + _L("Tips:") + "\n" +_L("\"Fix Model\" feature is currently only on Windows. Please repair the model on Pluraprint Slicer(windows) or CAD softwares."));
    }
    #endif //APPLE & LINUX
