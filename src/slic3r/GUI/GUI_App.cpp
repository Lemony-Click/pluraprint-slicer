...
/*#ifdef WIN32
    ...
    try {
        ...
        flush_logs();
        wxString errmsg = wxString::Format(_L("PluraprintSlicer will terminate because of running out of memory. "
        ...
        //throw;
     	...
     	BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        	flush_logs();
        wxString errmsg = _L("PluraprintSlicer will terminate because of a localization error. "
        ...
        BOOST_LOG_TRIVIAL(error) << boost::format("Uncaught exception: %1%") % ex.what();
        flush_logs();
        wxLogError(format_wxstr(_L("PluraprintSlicer got an unhandled exception: %1%"), ex.what()));
        throw;
    }
//#endif
}
...
{
	//app config initializes early becasuse it is used in instance checking in PluraprintSlicer.cpp
...
static boost::optional<Semver> parse_semver_from_ini(std::string path)
{
    ...
    size_t start = body.find("PluraprintSlicer ");
    if (start == std::string::npos) {
        start = body.find("PluraprintSlicer ");
        if (start == std::string::npos)
            return boost::none;
    }
...
void GUI_App::init_webview_runtime()
{
    ...
    if (!WebView::CheckWebViewRuntime()) {
        int nRet = wxMessageBox(_L("Pluraprint Slicer requires the Microsoft WebView2 Runtime to operate certain features.\nClick Yes to install it now."),
...
//	SetAppDisplayName(SLIC3R_APP_NAME);
    ...
    BOOST_LOG_TRIVIAL(info) << boost::format("gui mode, Current PluraprintSlicer Version %1% build %2%") % SoftFever_VERSION % GIT_COMMIT_HASH;
...
//     Slic3r::debugf "wxWidgets version %s, Wx version %s\n", wxVERSION_STRING, wxVERSION;
        ...
        if (!msg.empty() && !ssl_accept) {
            RichMessageDialog
                dlg(nullptr,
                    ...
                    "PluraprintSlicer", wxICON_QUESTION | wxYES_NO);
    ...
    if (is_editor()) {
        ...
        associate_url(L"pluraprint-slicer");
...
//#endif
    ...
    if (m_config_corrupted) {
        ...
        show_error(nullptr,
                   _u8L(
                       "The PluraprintSlicer configuration file may be corrupted and cannot be parsed.\nPluraprintSlicer has attempted to recreate the "
...
void GUI_App::on_http_error(wxCommandEvent &evt)
{
    ...
    if (code == HttpErrorVersionLimited) {
        MessageDialog msg_dlg(nullptr, _L("The version of Pluraprint Slicer is too low and needs to be updated to the latest version before it can be used normally."), "", wxAPPLY | wxOK);
        if (msg_dlg.ShowModal() == wxOK) {
        }
...
bool GUI_App::process_network_msg(std::string dev_id, std::string msg)
{
    if (dev_id.empty()) {
        if (msg == "wait_info") {
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Please try updating PluraprintSlicer and then try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("The certificate has expired. Please check the time settings or update PluraprintSlicer and try again."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            if (!m_show_error_msgdlg) {
                MessageDialog msg_dlg(nullptr, _L("Internal error. Please try upgrading the firmware and PluraprintSlicer version. If the issue persists, contact support."), "", wxAPPLY | wxOK);
                m_show_error_msgdlg = true;
                msg_dlg.ShowModal();
                m_show_error_msgdlg = false;
            }
            ...
            MessageDialog
                msg_dlg(nullptr,
                        _L("To use PluraprintSlicer with Bambu Lab printers, you need to enable LAN mode and Developer mode on your printer.\n\n"
                           "Please go to your printer's settings and:\n"
                           "1. Turn on LAN mode\n"
                           "2. Enable Developer mode\n\n"
                           "Developer mode allows the printer to work exclusively through local network access, "
                           "enabling full functionality with PluraprintSlicer."),
...
bool GUI_App::load_language(wxString language, bool initial)
{
    ...
    if (initial) {
        ...
        if (! language.empty())
        	BOOST_LOG_TRIVIAL(info) << boost::format("language provided by PluraprintSlicer.conf: %1%") % language;
        else {
	...
	if (language_info != nullptr && language_info->LayoutDirection == wxLayout_RightToLeft) {
    	BOOST_LOG_TRIVIAL(trace) << boost::format("The following language code requires right to left layout, which is not supported by PluraprintSlicer: %1%") % language_info->CanonicalName.ToUTF8().data();
		language_info = nullptr;
	}
    ...
    if (! wxLocale::IsAvailable(language_info->Language)) {
    	...
    	wxString message = "Switching Pluraprint Slicer to language " + language_info->CanonicalName + " failed.";
        ...
        wxMessageBox(message, "Pluraprint Slicer - Switching language failed", wxOK | wxICON_ERROR);
        if (initial)
...
void GUI_App::OSXStoreOpenFiles(const wxArrayString &fileNames)
{
    ...
    if (fileNames.size() == num_gcodes) {
        // Opening PrusaSlicer by drag & dropping a G-Code onto PluraprintSlicer icon in Finder,
...
void GUI_App::open_mall_page_dialog()
{
    ...
    if (link_url.find("?") != std::string::npos) {
        link_url += "&from=pluraprint-slicer";
    } else {
        link_url += "?from=pluraprint-slicer";
    }
...
void GUI_App::associate_files(std::wstring extend)
{
    ...
    std::wstring prog_id = L" Orca.Slicer.1";
    std::wstring prog_desc = L"PluraprintSlicer";
