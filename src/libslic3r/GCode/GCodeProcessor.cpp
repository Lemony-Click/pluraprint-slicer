...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: PluraprintSlicer is also "bambu". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-bambu
    { EProducer::PluraprintSlicer, SLIC3R_APP_NAME },
    { EProducer::PluraprintSlicer, "generated by PluraprintSlicer" },
    { EProducer::PluraprintSlicer, "generated by OrcaSlicer" },
    { EProducer::PluraprintSlicer, "generated by BambuStudio" },
    { EProducer::PluraprintSlicer, "BambuStudio" }
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by PluraprintSlicer,
        // extract the config from it
        if (m_producer == EProducer::PluraprintSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from PluraprintSlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::PluraprintSlicer) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::PluraprintSlicer: { return process_bambuslicer_tags(comment); }
...
// BambuStudio used extruder_id*2+mode to support per-nozzle limits, but PluraprintSlicer
