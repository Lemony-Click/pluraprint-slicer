...
public:
    ...
    bool load_opengl_dll()
    {
        ...
        wc.lpszClassName = L"PluraprintSlicer_opengl_version_check";
        wc.style = CS_OWNDC;
        if (RegisterClass(&wc)) {
            HWND hwnd = CreateWindowW(wc.lpszClassName, L"PluraprintSlicer_opengl_version_check", WS_OVERLAPPEDWINDOW, 0, 0, 640, 480, 0, 0, wc.hInstance, (LPVOID)this);
            if (hwnd) {
...
extern "C" {
    ...
    Slic3rMainFunc pluraprint_slicer_main = nullptr;
}
...
// http://download.qt.io/development_releases/prebuilt/llvmpipe/windows/
    ...
    wcscat(path_to_slic3r, L"PluraprintSlicer.dll");
//	printf("Loading Slic3r library: %S\n", path_to_slic3r);
    HINSTANCE hInstance_Slic3r = LoadLibraryExW(path_to_slic3r, nullptr, 0);
    if (hInstance_Slic3r == nullptr) {
        printf("PluraprintSlicer.dll was not loaded, error=%d\n", GetLastError());
        return -1;
    }

    // resolve function address here
    pluraprint_slicer_main = (Slic3rMainFunc)GetProcAddress(hInstance_Slic3r,
#ifdef _WIN64
        // there is just a single calling conversion, therefore no mangling of the function name.
        "pluraprint_slicer_main"
#else	// stdcall calling convention declaration
        "_bambustu_main@8"
#endif
        );
    if (pluraprint_slicer_main == nullptr) {
        printf("could not locate the function pluraprint_slicer_main in PluraprintSlicer.dll\n");
        return -1;
    }
    // argc minus the trailing nullptr of the argv
    return pluraprint_slicer_main((int)argv_extended.size() - 1, argv_extended.data());
}
}
